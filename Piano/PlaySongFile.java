...
public class PlaySongFile extends Thread implements Runnable{
    private MidiChannel channel,drum;
  private Scanner scan;
  private String songfile;
     FileInputStream stream;
     private String top;
     private int start, key, set = 0, ninth = 0, seventh = 0, inv = 0;
     boolean isSeventh = false, isNinth = false;
     ArrayList<Thread> threads = new ArrayList<Thread>();
     
     
  public PlaySongFile(String song,Synthesizer syn, String topper, int startLine, MidiChannel drummer) throws IOException {
    channel = syn.getChannels()[0];
    scan = new Scanner(new File(song));
...
for (int i = 0; i<count; i++){
    ...
    if (i>= start){
    if ((nextline.indexOf("isNotation") != -1|| nextline.indexOf("notNotation") != -1)&& top.equals("top")){
      try{
        Synthesizer sy = MidiSystem.getSynthesizer();
        sy.open();
 threads.add(new Thread(new PlaySongFile(songfile,sy, nextline,i,drum)));
     } catch (MidiUnavailableException po) {}
  }
    else if (top.indexOf("isNotation") != -1|| top.indexOf("notNotation") != -1){

      if (nextline.indexOf("isNotation") != -1||nextline.indexOf("notNotation") != -1){
       until++; 
       if (nextline.indexOf("isNotation") != -1)
        channel.programChange(Integer.parseInt(nextline.substring(nextline.indexOf("tion") + 5,
                                                                  nextline.indexOf(" ", nextline.indexOf(" ") + 1))),
                               Integer.parseInt(nextline.substring(nextline.indexOf(" ", nextline.indexOf(" ") + 1)+1)));
       }
      else{
      try{
        if (top.indexOf("isNotation") != -1){
        channel.allNotesOff();
        String frst = nextline.substring(0,nextline.indexOf("Hold")-5), vol = nextline.substring(nextline.indexOf("Hold")-5,nextline.indexOf("Hold")), scnd = nextline.substring(nextline.indexOf("Hold") +5 , nextline.length()-1);
        Scanner first = new Scanner(frst);
        Scanner volume = new Scanner(vol);
        int vols = volume.nextInt();
        while (first.hasNextInt()){
          channel.noteOn(first.nextInt(),vols);
        }
        Thread.sleep(Long.parseLong(scnd));
        channel.allNotesOff();}
        else if (top.indexOf("notNotation") != -1){
          if (nextline.indexOf("start") != -1){
            before = Long.parseLong(nextline.substring(nextline.indexOf("Hold") + 5, nextline.length()-1));
          key = Integer.parseInt(nextline.substring(6,nextline.indexOf("Hold"))) - 60;}
         else {
           Scanner read = new Scanner(nextline);
           int action = read.nextInt();
             if (action == -10000)
               channel.noteOff(key + 60);
             else if (action == -20000)channel.noteOff(key + 61); else {
                      switch (action){
       ...
       break;
        case 14:  drum.noteOn(set + 63, 100);
       break;
        case 15:  drum.noteOn(set + 64, 100); 
       break;
        case 16:  drum.noteOn(set + 65, 100);
       break;
        case 17:  drum.noteOn(set + 66, 100); 
       break;
        case 18:  drum.noteOn(set + 67, 100);
       break;
        case 19:  drum.noteOn(set + 68, 100); 
       break;
        case 20:  drum.noteOn(set + 69, 100); 
       break;
        case 21:  drum.noteOn(set + 70, 100); 
       break;
        case 22:  drum.noteOn(set + 71, 100); 
       break;
        case 23:  drum.noteOn(set + 72, 100); 
       break;
        case 24:  drum.noteOn(set + 73, 100); 
       break;
      ...
      {isSeventh = false;
       ...
       case 107: key++; break;
        ...
        Thread.sleep(Long.parseLong(nextline.substring(nextline.indexOf("Hold") + 5, nextline.length()-1))- before); before = Long.parseLong(nextline.substring(nextline.indexOf("Hold") + 5, nextline.length()-1));  }} catch (InterruptedException s) {}}
      if (until == 2 || ! scan.hasNextLine())
        break;
